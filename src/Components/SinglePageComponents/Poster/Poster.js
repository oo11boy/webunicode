...
export default function Poster({ singlepageid }) {
  ...
  const dynamicSinglePage = 
    singlepageid === 'store' ? [
      {
        title: 'قالب فروشگاهی',
        description: 'تیم قویه این طرحو با کمترین قیمت در اختیار شما قرار میده ددددد',
        content: 'سفارش بدید حالشو ببرید',
        images: [
          ...
          {dynamicSinglePage.map((page, index) => (
            <div key={index} className="flex lg:flex-row flex-col justify-between items-center w-full">
              <div className="lg:w-[40%] md:w-[60%] w-full space-y-5 pb-20 text-white">
                ...
                <h4 className="text-4xl font-bold text-start">{page.description}</h4>
                <h4 className="text-xl font-bold text-start">{page.content}</h4>
