...
const PostForm = React.memo(
  ({ isEditModalOpen, formData, error, categories, handleChange, onSubmit, onCancel }) => {
    const [imagePreview, setImagePreview] = useState(formData.mainimg || ""); // پیش‌نمایش تصویر
    const [uploading, setUploading] = useState(false); // وضعیت آپلود
    const [imageUrl, setImageUrl] = useState(formData.mainimg || ""); // لینک تصویر
    ...
    const handleImageUpload = async (e) => {
      ...
      try {
        ...
        if (res.ok) {
          handleChange({ target: { name: "mainimg", value: data.url } });
          setImagePreview(data.url);
          setImageUrl(`${data.url}`); // همگام‌سازی با لینک
        } else {
    ...
    const handleImageUrlChange = (e) => {
      ...
      setImagePreview(url); // نمایش پیش‌نمایش از لینک
    };
              ...
              {imagePreview && (
                <div className="mt-2 relative border border-gray-200 rounded-lg p-2 bg-gray-50 shadow-md">
                  <img
                    ...
                    onError={() => setImagePreview("")} // در صورت خطا در بارگذاری تصویر
                  />
                  <button
                    type="button"
...
export default function PostsPage() {
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchPosts();
        setFormData(initialPostFormData); // ریست فرم
      } else {
        const data = await res.json();
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        fetchPosts();
        setFormData(initialPostFormData); // ریست فرم بعد از ویرایش موفق
      } else {
        const data = await res.json();
        setError(data.error || "خطایی رخ داد");
      }
  ...
  const handleCancel = () => {
    setFormData(initialPostFormData); // ریست فرم هنگام انصراف
    setIsAddModalOpen(false);
    setIsEditModalOpen(false);
  };
    ...
    <>
      <div className="flex-1 p-4 md:p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={() => {
              setFormData(initialPostFormData); // ریست فرم قبل از باز کردن مودال افزودن
