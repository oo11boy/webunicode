...
const SEOAnalyzer = ({ formData }) => {
  ...
  const analyzeSEO = () => {
    ...
    const keywordInMetaTitle = keyword && formData.metatitle?.toLowerCase().includes(keyword);
    if (metaTitleLength >= 50 && metaTitleLength <= 60 && keywordInMetaTitle) {
      score += 15;
    } else {
      if (metaTitleLength < 50 || metaTitleLength > 60) {
        newIssues.push(`طول عنوان متا ${metaTitleLength} کاراکتر است. باید بین 50 تا 60 کاراکتر باشد.`);
      ...
      if (!keywordInMetaTitle && keyword) {
        ...
        newSuggestions.push(`کلمه کلیدی "${keyword}" را در ابتدای عنوان متا قرار دهید.`);
      }
    }

    // 2. بررسی توضیحات متا (15 امتیاز)
    const metaDescriptionLength = formData.metadescription?.length || 0;
    const keywordInMetaDescription = keyword && formData.metadescription?.toLowerCase().includes(keyword);
    if (metaDescriptionLength >= 150 && metaDescriptionLength <= 160 && keywordInMetaDescription) {
      score += 15;
    } else {
      if (metaDescriptionLength < 150 || metaDescriptionLength > 160) {
        newIssues.push(`طول توضیحات متا ${metaDescriptionLength} کاراکتر است. باید بین 150 تا 160 کاراکتر باشد.`);
      ...
      if (!keywordInMetaDescription && keyword) {
        ...
        newSuggestions.push(`کلمه کلیدی "${keyword}" را در توضیحات متا قرار دهید.`);
      }
    }

    // 3. بررسی کلمه کلیدی در متن (15 امتیاز)
    const keywordCount = keyword ? (plainText.toLowerCase().match(new RegExp(keyword, "g")) || []).length : 0;
    ...
    if (keywordDensity >= 1 && keywordDensity <= 2 && keywordInFirst100) {
      ...
      if (keywordDensity < 1 || keywordDensity > 2) {
        newIssues.push(`تراکم کلمه کلیدی (${keywordDensity.toFixed(2)}%) در متن اصلی مناسب نیست. باید بین 1-2% باشد.`);
        newSuggestions.push("تعداد استفاده از کلمه کلیدی را تنظیم کنید تا تراکم بین 1-2% باشد.");
      }
      if (!keywordInFirst100 && keyword) {
        newIssues.push("کلمه کلیدی در 100 کلمه اول متن وجود ندارد.");
        newSuggestions.push(`کلمه کلیدی "${keyword}" را در 100 کلمه اول متن قرار دهید.`);
      }
    }
    ...
    if (!hasH1InEditor && h2Count >= 1 && h2Count <= 10) {
      ...
      if (hasH1InEditor) {
        ...
        newSuggestions.push("تگ H1 را از متن ادیتور حذف کنید و از H2 یا H3 استفاده کنید.");
      }
      if (h2Count === 0) {
      ...
      if (!hasH3orH4) {
        newSuggestions.push("از هدینگ‌های H3 یا H4 برای زیربخش‌ها استفاده کنید.");
      }
    }
    ...
    if (hasImageInText && !hasMissingAlt && (formData.mainimg || textContent.includes("<img"))) {
      ...
      if (!isWebPFormat && formData.mainimg) {
        newSuggestions.push("برای بهبود سرعت بارگذاری، از فرمت WebP برای تصویر اصلی استفاده کنید.");
      }
    }
    ...
    if (hasInternalLink && hasExternalLink) {
      ...
      if (!hasInternalLink) {
        ...
        newSuggestions.push(`حداقل یک لینک داخلی به آدرس‌های مرتبط با ${hostname} اضافه کنید.`);
      }
      if (!hasExternalLink) {
        newIssues.push("لینک خارجی وجود ندارد.");
        newSuggestions.push("حداقل یک لینک خارجی معتبر به منابع مرتبط اضافه کنید.");
      }
      if (hasExternalLink && !hasNofollowExternal) {
        newSuggestions.push("برای لینک‌های خارجی غیرمعتبر، از rel='nofollow' استفاده کنید.");
      }
    }
    ...
    if (isValidSlug) {
      ...
      newSuggestions.push(`URL باید کوتاه، توصیفی، شامل کلمه کلیدی "${keyword}" و فقط حاوی حروف، اعداد و خط تیره باشد.`);
    }

    // 8. بررسی طول محتوا و UX (15 امتیاز)
    const hasShortParagraphs = plainText.split("\n").every((para) => para.split(/\s+/).length <= 50);
    const hasLists = textContent.includes("<ul") || textContent.includes("<ol");
    const hasCTA = plainText.toLowerCase().includes("بیشتر بخوانید") || plainText.toLowerCase().includes("اکنون") || plainText.toLowerCase().includes("تماس بگیرید");
    if (wordCount >= 600 && hasShortParagraphs && hasLists && hasCTA) {
      score += 15;
    } else {
      if (wordCount < 600) {
        newIssues.push(`محتوای اصلی ${wordCount} کلمه است. باید حداقل 600 کلمه باشد.`);
      ...
      if (!hasLists) {
        newSuggestions.push("از لیست‌های شماره‌دار یا بولت‌دار برای خوانایی استفاده کنید.");
      }
      if (!hasCTA) {
        newSuggestions.push("حداقل یک Call-to-Action (مثل 'بیشتر بخوانید') اضافه کنید.");
      }
    }

    // 9. بررسی داده‌های ساختارمند (5 امتیاز)
    const hasSchema = textContent.includes('application/ld+json') || textContent.includes('itemscope');
    if (hasSchema) {
      score += 5;
    } else {
      newSuggestions.push("داده‌های ساختارمند (مانند Schema Article یا FAQ) اضافه کنید.");
    }
    ...
    if (shortTextLength >= 100 && shortTextLength <= 200) {
      ...
      newIssues.push(`طول متن کوتاه ${shortTextLength} کاراکتر است. باید بین 100 تا 200 کاراکتر باشد.`);
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg border border-gray-200 p-6 mt-6">
      ...
      {/* نمایش امتیاز سئو */}
      <div className="mb-4">
        <label className="block text-sm font-medium text-gray-700">امتیاز سئو</label>
      ...
      {suggestions.length > 0 && (
        <div>
          <h3 className="text-md font-semibold text-blue-600">پیشنهادات بهبود</h3>
          <ul className="list-disc list-inside text-sm text-gray-700">
