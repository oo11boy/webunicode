...
export default function SlideCard({ items }) {
  ...
  return (
    <div className="w-full shadow-md my-10 slidecard relative bg-white rounded-xl overflow-hidden h-[400px]">
      ...
      <ImageWithLoader height={'!h-[65%]'} src={items.img} alt={items.name} className={"h-[50%] w-full"}/>
      <a
        target="_blank"
      ...
      >
        <div className="skew-x-11 icontitletext flex w-[40%] bg-red-700 justify-center items-center h-full">
          <span
            className={`${items.icon} text-2xl -skew-x-12 text-white pr-4`}
          ></span>
        </div>
        <div className="flex justify-center items-center -skew-x-12 pr-4 text-xl">
          <div href="#" className="text-white text-lg titletext">
            {items.name}
          </div>
        </div>
      </a>
      
      <p className="p-4 opacity-60 mt-16 h-[30%]">{items.content}</p>

      <a
        target="_blank"
        href={`../viewportfolio/${items.id}`}
        className={`portfolio-slider-link flex items-center px-4 ${
          hoverlink ? "hover" : ""
        }`}
      >
